...
# Get 2025 components (absolute numbers)
...
# Use absolute numbers (in thousands for readability)
states['natural_k'] = states['natural'] / 1000
states['domestic_k'] = states['domestic'] / 1000
states['international_k'] = states['international'] / 1000
states['total_k'] = states['total'] / 1000

# Set bucket boundaries for components (natural, domestic, international)
# Boundaries in thousands
component_boundaries = [-np.inf, -100, -25, 25, 100, np.inf]
print(f"Component boundaries (thousands): <-100, -100 to -25, -25 to 25, 25 to 100, >100")
component_norm = mcolors.BoundaryNorm(component_boundaries, diverging_cmap.N)

# Set bucket boundaries for total (wider range)
total_boundaries = [-np.inf, -50, -10, 50, 150, np.inf]
print(f"Total boundaries (thousands): <-50, -50 to -10, -10 to 50, 50 to 150, >150")
total_norm = mcolors.BoundaryNorm(total_boundaries, diverging_cmap.N)
...
gdf = gdf.merge(states[['STATEFP', 'natural_k', 'domestic_k',
                         'international_k', 'total_k', 'NAME']],
                on='STATEFP', how='left')
...
maps = [
    ('natural_k', 'Natural Change\n(births minus deaths)', axes[0, 0], component_norm),
    ('domestic_k', 'Domestic Migration', axes[0, 1], component_norm),
    ('international_k', 'International Migration', axes[1, 0], component_norm),
    ('total_k', 'Total Population Change', axes[1, 1], total_norm),
]
...
# Add two legends - one for components, one for total

# Components legend (left)
cbar_ax1 = fig.add_axes([0.1, 0.04, 0.35, 0.02])
cbar_ax1.set_facecolor('#F6F7F3')
for i, color in enumerate(discrete_colors):
    cbar_ax1.add_patch(plt.Rectangle((i/5, 0), 1/5, 1, facecolor=color, edgecolor='white', linewidth=0.5))
cbar_ax1.set_xlim(0, 1)
cbar_ax1.set_ylim(0, 1)
cbar_ax1.axis('off')
cbar_ax1.text(0.5, 1.5, 'Components (thousands)', ha='center', va='bottom', fontsize=9, fontweight='bold', color='#3D3733')
for pos, label in zip([0.1, 0.3, 0.5, 0.7, 0.9], ['<-100', '-100\nto -25', '-25\nto +25', '+25\nto +100', '>+100']):
    cbar_ax1.text(pos, -0.4, label, ha='center', va='top', fontsize=7, color='#3D3733')

# Total legend (right)
cbar_ax2 = fig.add_axes([0.55, 0.04, 0.35, 0.02])
cbar_ax2.set_facecolor('#F6F7F3')
for i, color in enumerate(discrete_colors):
    cbar_ax2.add_patch(plt.Rectangle((i/5, 0), 1/5, 1, facecolor=color, edgecolor='white', linewidth=0.5))
cbar_ax2.set_xlim(0, 1)
cbar_ax2.set_ylim(0, 1)
cbar_ax2.axis('off')
cbar_ax2.text(0.5, 1.5, 'Total (thousands)', ha='center', va='bottom', fontsize=9, fontweight='bold', color='#3D3733')
for pos, label in zip([0.1, 0.3, 0.5, 0.7, 0.9], ['<-50', '-50\nto -10', '-10\nto +50', '+50\nto +150', '>+150']):
    cbar_ax2.text(pos, -0.4, label, ha='center', va='top', fontsize=7, color='#3D3733')
...
plt.savefig(OUTPUT_DIR / "migration_components_small_multiples.png",
            facecolor='#F6F7F3', bbox_inches='tight', dpi=150)
plt.savefig(OUTPUT_DIR / "migration_components_small_multiples.svg",
            facecolor='#F6F7F3', bbox_inches='tight')

print(f"\nSaved to {OUTPUT_DIR / 'migration_components_small_multiples.png'}")
