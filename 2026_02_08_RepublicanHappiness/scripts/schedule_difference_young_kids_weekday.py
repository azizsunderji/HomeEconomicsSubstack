...
bin_df_parent = con.execute(f"""
    ...
    exploded AS (
        SELECT act_cat, ((start_min + slot_offset) % 1440) // 10 as bin_idx, wt
...
total_parent = con.execute(f"""
    ...
    exploded AS (
        SELECT ((start_min + slot_offset) % 1440) // 10 as bin_idx, wt
        FROM activities, UNNEST(GENERATE_SERIES(0, dur - 1, 10)) as t(slot_offset)
    )
...
sleep_deficit_per_bin = shares['Sleep'] / 100 * 10  # minutes lost per 10-min bin
...
# Title
ax.set_title('The parent time swap: employed parents of young kids', fontsize=16, fontweight='bold',
...
# Source
fig.text(0.05, 0.02, f'Source: ATUS 2003-2023, ages 25-54, weekdays, employed (n={n_parent:,} parents, {n_nonparent:,} non-parents)',
...
# Save
outpath = '/Users/azizsunderji/Dropbox/Home Economics/2026_02_08_RepublicanHappiness/outputs/schedule_difference_young_kids_weekday.svg'
