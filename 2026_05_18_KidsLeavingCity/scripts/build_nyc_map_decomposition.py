"""
Interactive NYC MSA map + decomposition chart.

Layout: Map of 22 NYC MSA counties on the left (color-coded by net change in
under-18, 2010-2024). Decomposition chart on the right that updates on hover —
shows the same per-county cumulative components view (natural, dom mig, intl mig)
for whatever county the cursor is on.

Default state: shows MSA-aggregate decomposition.

Inputs:
  data/geo/metro10_counties.geojson    (NYC counties subset)
  data/nyc_county_under18_decomposition.csv  (annual components 2010-2024)
  data/nyc_county_under18_pep_seamless.csv   (1980-2024 level — for context)

Output: outputs/nyc_decomposition_map.html
"""
import json
...
for fips in NYC_FIPS:
    ...
    all_net_change.append(counties[fips]["net_change"])

# Color scale: diverging on net change 2010-2024
...
<body>
<h1>NYC MSA under-18: hover a county to decompose the change</h1>
<div class="sub">Map shaded by net change in under-18 since 2010. Blue = gained kids; red = lost kids. Hover any county to see the trajectory 1980-2024 and the cumulative decomposition since 2010 (natural change, domestic migration, international migration). Default view shows the MSA-aggregate decomposition.</div>

<div class="layout">
  <div class="map-wrap">
    <svg id="map" viewBox="0 0 600 540" width="100%" height="auto"></svg>
    <div class="legend">
      <span>−__MAX_K__K kids</span>
      <span class="legend-bar"></span>
      <span>+__MAX_K__K kids</span>
      <span style="margin-left:20px;font-size:11px">(net change since 2010)</span>
    </div>
  </div>
...
const paths = mapSvg.append('g').selectAll('path').data(P.geo.features).join('path')
  ...
  .attr('fill', d => {
    ...
    return c ? colorScale(c.net_change) : '#eee';
  })
...
mapSvg.append('g').selectAll('text').data(P.geo.features).join('text')
  .attr('class', d => {
    const c = counties[d.properties.county_fips];
    const intensity = c ? Math.abs(c.net_change) / maxAbs : 0;
...
html = (template.replace("__DATA__", json.dumps(P))
                .replace("__MAX_K__", f"{int(round(max_abs_change/1000))}"))
out_path = OUT / "nyc_decomposition_map.html"
out_path.write_text(html)
print(f"Wrote {out_path} ({out_path.stat().st_size/1e6:.2f} MB)")
print(f"Max abs net change: {max_abs_change:,}")
